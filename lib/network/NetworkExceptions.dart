...
abstract class NetworkExceptions with _$NetworkExceptions {
  ...
  static String getErrorMessage(NetworkExceptions networkExceptions) {
    ...
    networkExceptions.when(
      ...
      internalServerError: () {
        errorMessage = "Oh no! Something bad happened. Please come back later when we've fixed that problem.";
      },
      ...
      unProcessableEntity: (dynamic e) {
        var error = (e['errors'] as Map).map((key, value) => MapEntry(key, (value as List).map<String>((e) => e).toList()));
        if (error['email']![0] == 'The provided credentials are incorrect.') {
          errorMessage = 'The email or password you entered did not match out records. Please double-check and try again.';
        } else {
          errorMessage = '';
        }
      },
